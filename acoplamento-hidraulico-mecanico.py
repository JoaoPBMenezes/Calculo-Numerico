...
Observação sobre custo computacional:
    ...
    nos casos 101x101. Use --quick para gerar gráficos de apresentação rápidos.
"""
...
class Config:
    ...
    outlet_node: int = 5
    mu: float = 5e-4                         # Pa.s
    channel_width: float = 1000e-6           # m
    network_length_unit: float = 1e-3        # 1 unidade do grafo = 1 mm

    # Membrana
    radius: float = 0.0025                   # m = 0.25 cm
    thickness: float = 0.0001                # m = 0.1 mm
    sigma: float = 200.0                     # N/m
    rho: float = 900.0                       # kg/m^3
    beta_hat: float = 0.1                    # amortecimento adimensional
    Nx: int = 51
    Ny: int = 51

    # Tempo ADIMENSIONAL: tau = t/t_ref
    dt_hat: float = 0.025
    tau_final: float = 12.0

    # Entrada
    p_inlet: float = 5000.0                  # Pa

    # Visualização / saída
    output_dir: str = "resultados_acoplamento"
    show_plots: bool = False

    # Amplitude inicial para rotinas modais, em unidades de w_ref
    modal_initial_amplitude_hat: float = 0.5


def reference_scales(cfg: Config) -> Dict[str, float]:
    """Escalas do PDF: L_ref=R, w_ref=0.01R, t_ref=R*sqrt(rho*e/sigma), p_ref=sigma*w0/R^2."""
    R = cfg.radius
    w_ref = 0.01 * R
...
def generate_graph_arrays(levels: int = 3) -> Tuple[np.ndarray, np.ndarray]:
    """Gera o grafo fractal usado pela rotina original, mantendo inlet=0 e outlet=5."""
...
def edge_lengths(Xno: np.ndarray, conec: np.ndarray, length_unit: float) -> np.ndarray:
    """Comprimento físico dos canais em metros."""
...
def hydraulic_conductivities(Xno: np.ndarray, conec: np.ndarray, mu: float, width: float, length_unit: float) -> Dict[str, np.ndarray]:
    """Condutância de cada canal, usando diâmetro hidráulico equivalente da área quadrada."""
...
def impose_pressure_bc_on_A(A: sparse.csr_matrix, node: int) -> sparse.csr_matrix:
    """Impõe pressão prescrita no nó por substituição da linha: p_node = valor_RHS."""
    A = A.tolil(copy=True)
    A[node, :] = 0.0
    A[node, node] = 1.0
    return A.tocsr()
...
def assembly_membrane(Nx: int, Ny: int, Lx_hat: float = 2.0, Ly_hat: float = 2.0, big_number: float = 1e8) -> Tuple[sparse.csr_matrix, sparse.csr_matrix, np.ndarray, np.ndarray, float, np.ndarray]:
    """Monta K e M adimensionais para -laplaciano em membrana circular fixa."""
...
def solve_membrane_modes(K: sparse.csr_matrix, M: sparse.csr_matrix, num_modes: int = 8) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Autovalores K phi = lambda M phi; omega_hat=sqrt(lambda)."""
    n = K.shape[0]
    k = min(num_modes, n - 2)
...
def run_simulation(
    ...
    """Resolve o sistema acoplado no tempo adimensional tau=t/t_ref."""
    ...
    for k, tau_k in enumerate(tau):
        ...
        rhs[:n_m] = w / cfg.dt_hat
        rhs[n_m:2 * n_m] = M.dot(v) / cfg.dt_hat

        p_in = forcing_func(float(tau_k), float(t_phys[k]), scales) if forcing_func else cfg.p_inlet
        rhs[2 * n_m + cfg.inlet_node] = p_in / scales["p_ref"]
        ...
        # volume físico: dV = q * dt_físico = q * dt_hat * t_ref
        ...
        hist["volume"][k] = vol
        hist["power"][k] = p_in * q_out  # potência fornecida na entrada; comparar também p_out*q_out se desejar
...
def rotina_1_matriz_R(cfg: Config) -> None:
    ...
    # Para visualizar R = h^2 U^T A^{-1} U, resolve A X = U em vez de formar inversa explicitamente.
...
def rotina_2_evolucao_parametrica(cfg: Config, full_sweep: bool = False) -> None:
    ...
    # Caso-base para os seis gráficos exigidos e perfil transiente.
    ...
    # Comparações pedidas pelo PDF, com curvas selecionadas para ficarem legíveis.
    curves_p = []
    ...
    # Para dt diferentes, cada curva tem tau próprio; plota individualmente no mesmo eixo.
    fig, ax = plt.subplots(figsize=(8.5, 5.0))
...
def rotina_3_queda_pressao(cfg: Config) -> None:
    ...
    # Exatamente como pedido: 51x51 e dt_hat=0.025. Usa configuração final do item 2 como estado inicial.
    base = replace(cfg, Nx=51, Ny=51, dt_hat=0.025, tau_final=12.0,
                   p_inlet=5000.0, channel_width=1000e-6, beta_hat=0.1)
...
def estimate_frequency_from_zero_crossings(tau: np.ndarray, y: np.ndarray) -> Tuple[float, float]:
    ...
    # interpolação linear dos cruzamentos
    crossings = []
    for i in idx:
        t0, t1 = tau[i], tau[i + 1]
        y0, y1 = y[i], y[i + 1]
...
def rotina_5_forcamento_harmonico(cfg: Config) -> None:
    ...
    def forcing(tau_hat: float, t_phys: float, scales: Dict[str, float]) -> float:
        # Coerente com a formulação adimensional: p_inlet(tau)=5000*cos(omega3_hat*tau)
